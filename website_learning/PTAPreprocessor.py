...
class PTAPreprocessor:
    ...
    def __call__(self, root: GsmNode) -> GsmNode:
        ...
        if len(nodes) < 100:
            # this is slow, only for small systems
            root.visualize(Util.base_dir / RESULTS_DIRECTORY_NAME / "PTA_before_preprocessing")
        ...
        for node in nodes:
            ...
            for letter in Util.input_alphabet:
                if letter.startswith(current_page):
                    ...
                    node.transitions[letter] = {BOUNDARY_OF_SCOPE_STR + current_page:
                                                TransitionInfo(child_node, 1, child_node, 1)}
                elif not Util.webpage_has_interactive_elements[current_page]:
                    child_node = GsmNode((letter, DEAD_END_STR + current_page), node)
                    node.transitions[letter] = {DEAD_END_STR + current_page:
                                                TransitionInfo(child_node, 1, child_node, 1)}
                else:
                    ...
                    node.transitions[letter] = {child_output:
                                                TransitionInfo(child_node, 1, child_node, 1)}
                    ...
                    if Settings.input_enabled_handling == InputEnabledHandling.SINK_STATE:
                        for inp in Util.input_alphabet:
                            ...
                            child_node.transitions[inp] = {STAY_SINK_STR:
                                                           TransitionInfo(grandchild_node, 1, grandchild_node, 1)}
                            Util.logger.debug("added a new grandchild node for " + STAY_SINK_STR)
        nodes = root.get_all_nodes()
        if len(nodes) < 100:
            # this is slow, only for small systems
            root.visualize(Util.base_dir / RESULTS_DIRECTORY_NAME / "PTA_after_preprocessing")
